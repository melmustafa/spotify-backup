...
async function getPlaylist(access_token, playlist_id) {
  ...
  let response = await fetchJson(
    `https://api.spotify.com/v1/playlists/${playlist_id}/tracks?fields=next,items%28track%28name%2Calbum%28name%29%2Cartists%28name%29%29%29&locale=en`,
    opts,
  );
...
function backupPlaylist(name, track_list) {
  let data = "name, album, artist\n";
  for (const t of track_list) {
    data = data + `"${t.name}", "${t.album}", "${t.artist}"\n`;
  }
  fs.writeFile(`${name}.csv`, data, "utf-8", (err) => {
    if (err) {
      console.error(err);
    }
  });
...
async function backUserPlaylists(user_id) {
  ...
  for (const list of playlists) {
    const tracks = await getPlaylistSongs(list.id);
